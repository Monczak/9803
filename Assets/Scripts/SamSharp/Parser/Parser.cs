            ...
            for (int i = 0; i < phonemeIndexes.Count; i++)
            {
                if (!skipEmpty || (phonemeIndexes[i] != null && phonemeIndexes[i] != 0))
                    result.Add(new PhonemeData(
                        ...
                        phonemeLengths[i], 
                        stresses[i],
                        i > 0 && ((phonemeIndexes[i - 1] == null || phonemeIndexes[i - 1] == 0 || phonemeIndexes[i - 1] == 4) && phonemeIndexes[i] != 4 && phonemeIndexes[i] != 81),
                        phonemeIndexes[i] > 4 && (!phonemeIndexes.ContainsKey(i + 1) || phonemeIndexes[i + 1] <= 4) && phonemeIndexes[i] != 81
                    ));
            }
